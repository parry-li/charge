...
/**
 * Created by PARRY on 17.04.08.
 */
public class LogUtil {
    public static String customTagPrefix = "parry";  // 自定义Tag的前缀，可以是作者名
    ...
    public static void i(String mTag, String content) {
        ...
        callerClazzName = callerClazzName.substring(callerClazzName.lastIndexOf(".") + 1);
        tag = String.format(tag, callerClazzName, caller.getMethodName(), caller.getLineNumber()); // 替换
        String lgotag = TextUtils.isEmpty(mTag) ? tag : mTag + ":" + tag;


        if (customLogger != null) {
            customLogger.i(lgotag, content);
        } else {
            Log.i(lgotag, content);
        }
        if (isSaveLog) {
            point(LOG_PATH, lgotag, content);
        }
    }
